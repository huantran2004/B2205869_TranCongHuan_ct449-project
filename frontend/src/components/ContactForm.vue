<template>
  <!-- schema kiểm tra hợp lệ bằng yup -->
  <Form @submit="submitContact" :validation-schema="contactFormSchema">
    ...
    <div class="form-group">
      ...
      <!-- Chỉ hiện nút Xóa khi có _id (chế độ edit) -->
      <button
...
export default {
  ...
  data() {
    const contactFormSchema = yup.object().shape({
      ...
      phone: yup
        .string()
        /* mẫu số ĐT Việt Nam như tài liệu minh họa */
        .matches(/((09|03|07|08|05)+([0-9]{8}))/g, "Số điện thoại không hợp lệ."),
    });

    return {
      // Không chỉnh trực tiếp props → tạo biến cục bộ để binding với Field
  ...
  methods: {
    ...
    deleteContact() {
      this.$emit("delete:contact", this.contactLocal.id || this.contactLocal._id);
    },
    Cancel() {
